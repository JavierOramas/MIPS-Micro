...
rtype_1_re  = re.compile(r'''^(?P<instr>(pop|push|jr|mfhi|mflo|tty|rnd|kbd))\s+(?P<r>r([0-9]|[1-2][0-9]|30|31))$''')
rtype_2_re  = re.compile(r'''^(?P<instr>(mult|mulu|div|divu))\s+(?P<rs>r([0-9]|[1-2][0-9]|30|31))\s+(?P<rt>r([0-9]|[1-2][0-9]|30|31))$''')
rtype_3_re  = re.compile(r'''^(?P<instr>(add|sub|slt|sltu|and|or|nor|xor))\s+(?P<rd>r([0-9]|[1-2][0-9]|30|31))\s+(?P<rs>r([0-9]|[1-2][0-9]|30|31))\s+(?P<rt>r([0-9]|[1-2][0-9]|30|31))$''')

itype_1_re  = re.compile(r'''^(?P<instr>(blez|bgtz|bltz))\s+(?P<rs>r([0-9]|[1-2][0-9]|30|31))\s+(?P<label>\w+)$''')
itype_2_re  = re.compile(r'''^(?P<instr>(addi|slti|sltiu|andi|ori|xori))\s+(?P<rt>r([0-9]|[1-2][0-9]|30|31))\s+(?P<rs>r([0-9]|[1-2][0-9]|30|31))\s+(?P<immed>-?(0x)?[0-9a-fA-F]+)$''')
branch_re   = re.compile(r'''^(?P<instr>(beq|bne))\s+(?P<rs>r([0-9]|[1-2][0-9]|30|31))\s+(?P<rt>r([0-9]|[1-2][0-9]|30|31))\s+(?P<label>\w+)$''')

mem_re      = re.compile(r'''^(?P<instr>(lw|sw))\s+(?P<rt>r([0-9]|[1-2][0-9]|30|31))\s+(?P<immed>-?(0x)?[0-9a-fA-F]+)\s*\(\s*(?P<rs>r([0-9]|[1-2][0-9]|30|31))\s*\)$''')
  ...
  for line in inputFile:
    ...
    if len(instruction) != 0:
      ...
      if rtype_0:        
        ...
        debug("{0:s} hex_code: {2:04x}\n{1:s}\n".format( instruction, pprintInstr([6,11,16,21,26], num), num))
      elif rtype_1:
        ...
        num = opcode << 26 | rs << 21 | rt << 16 | rd << 11 | funct
        debug("{0:s} rtype: rs: {1:d} rt: {2:d} rd: {3:d} funct: {4:d} hex_code: {6:04x}\n{5:s}\n".format( instruction,rs,rt,rd,funct,pprintInstr([6,11,16,21,26],num),num ))
      elif rtype_2:
        ...
        signed = instr in ["addi", "slti", "lw", "sw", "beq", "bne", "blez", "bgtz"]
        ...
        debug("{0:s} itype: rs: {1:d} rt: {2:d} hex_code: {4:04x}\n{3:s}\n".format( instruction,rs,rt,pprintInstr([6,11,16],num),num ))
      elif itype_1:
        ...
        debug("{0:s} itype: rs: {1:d} offset: {4:d} hex_code: {3:04x}\n{2:s}\n".format( instruction,rs,pprintInstr([6,11,16],num),num, offset ))
        ...
        if offset > 2 ** 15 - 1 or offset < -(2 ** 15):
          raise AssemblerRangeError(lineNo,"label %s is too far away: %d instructions from pc+1" % (label,offset))
        num = opcode << 26 | rs << 21 | rt << 16 | (offset & 65535)
        debug("{0:s} rs: {1:d} rt: {2:d} opcode: {3:d} offset: {4:d} hex_code: {5:04x}\n{6:s}\n".format(instruction, rs, rt, opcode, offset, num, pprintInstr([6,11,16],num)))
        ...
        debug("{0:s} addr: {1:d} hex_code: {2:04x}\n{3:s}\n".format(instruction,instructionNo ,num, pprintInstr([6],num) ))
...
def print_instructions(instructions, outputdir):
  ...
  little_endian = [ inst[6:8]+inst[4:6]+inst[2:4]+inst[0:2] for inst in hex_instructions]
...
if __name__ == "__main__":
  ...
  parser.add_option('-o','--out',dest='output_folder',type='string',
                    default='.',help="Specify output folder to write the 4 memory bank dumps.")
  ...
  try:
    ...
    print("Unable to open input file %s" % input_file, file=sys.stderr)
    sys.exit(1)
  try:
  ...
  try:
    ...
    print("Unable to write to output file %s" % output_file, file=sys.stderr)
    sys.exit(1)
  sys.exit(0)